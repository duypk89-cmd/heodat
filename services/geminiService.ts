...
const AI_OVERLOAD_MESSAGE = "Thỏ Mây đang quá tải vì nhiều việc quá! 🤯 Mây quay lại sau chút xíu nhé!";
...
export const getShoppingAdvice = async (items: string[]) => {
  try {
    ...
    console.error("Shopping Advice Error:", e);
    return AI_OVERLOAD_MESSAGE;
  }
};
...
export const suggestDailyMenu = async (style: string) => {
  try {
    ...
    console.error("Suggest Menu Error:", e);
    // Return fallback menu when API quota is exhausted
    return {
      tips: "Thỏ Mây đang quá tải nên gợi ý thực đơn mẫu cho Mây nè! (AI đang bận 🤯)",
      breakfast: { dish: "Bánh mì ốp la", ingredients: ["Bánh mì", "Trứng gà", "Dưa leo", "Nước tương"] },
      lunch: { dish: "Thịt kho trứng cút", ingredients: ["Thịt ba chỉ", "Trứng cút", "Nước dừa", "Hành tím"] },
      dinner: { dish: "Canh rau ngót thịt bằm", ingredients: ["Rau ngót", "Thịt bằm", "Gia vị"] }
    };
  }
};
